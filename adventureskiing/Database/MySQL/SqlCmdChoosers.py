...
class EarningsCmdChooser(object):
    ...
    def get_daily_earnings_cmd(self, period, user):
        ...
        cmd = "select sum(koszt) from lekcja where data = STR_TO_DATE('{}', '%Y/%m/%d') ".format(date)
        if user:
    ...
    def get_monthly_earnings_cmd(self, period, user):
        ...
        cmd = "Select sum(koszt) from lekcja where data between STR_TO_DATE('{month}/1', '%Y/%m/%d') " \
              "and STR_TO_DATE('{month}/{month_length}', '%Y/%m/%d') ".format(month=period[1],
    ...
    def get_season_earnings_cmd(self, period, user):
        cmd = "Select sum(koszt) from lekcja where data between STR_TO_DATE('{}/8/1', '%Y/%m/%d') " \
              "and STR_TO_DATE('{}/6/1', '%Y/%m/%d') ".format(period[1].split("/")[0], period[1].split("/")[1])
        if user:
            cmd += "and id_instruktora = {}".format(user.id)
        return cmd
    ...
    def get_weekly_earnings_cmd(self, period, user):
        ...
        cmd = "Select sum(koszt) from lekcja where data between " \
              "STR_TO_DATE('{}', '%Y/%m/%d') and STR_TO_DATE('{}', '%Y/%m/%d') ".format(start_date, end_date)
        if user:
            cmd += "and id_instruktora = {}".format(user.id)
        return cmd
